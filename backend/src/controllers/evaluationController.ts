/**
 * 평가 컨트롤러
 * @description LLM 평가 요청 처리
 */
import { Request, Response } from 'express';
import { evaluateWithLLM } from '../services/llmService';
import xss from 'xss';
 ...
 */
export async function evaluateReport(req: Request, res: Response): Promise<void> {
  try {
    ...
    const results = await Promise.all(files.map(async (file) => {
      try {
        const evalResultArr = await evaluateWithLLM(file, prompt, model); // 항상 배열 반환
